...
author: "Christine D. Sprunger, Steven W. Culman, Ariane L. Peralta, Tiana DuPont, Jay T. Lennon, Sieglinde S. Snapp"
...
## Nematode Data
```{r Import Files - Nema, include=FALSE}
# Import Environmental Data
...
write.csv(diversity,"diversity.bact.raw.csv")
```
...
library(lmerTest)
richness.lm <- lmer(richness ~ Date*Management*Plant + (1|Block), data = diversity)
plot(richness.lm)
richness.lm
anova(richness.lm)

richness.lm2 <- lmer(richness ~ Management*Plant + (1|Block), data = diversity)
plot(richness.lm2)
richness.lm2
anova(richness.lm2)

evenness.lm <- lmer(simp.even ~ Date*Management*Plant + (1|Block), data = diversity)
plot(evenness.lm)
evenness.lm
anova(evenness.lm)

shannon.lm <- lmer(shannon ~ Date*Management*Plant + (1|Block), data = diversity)
plot(shannon.lm)
shannon.lm
anova(shannon.lm)
...
summary <- diversity %>% group_by(Management, Plant, Year) %>% summarise(mean.richness=mean(richness), se.richness=se(richness), mean.shannon=mean(shannon), se.shannon=se(shannon))
print(summary)

write.csv(summary,"diversity.bact.summary.csv")
```
...
```{r Ordination (PCoA) - Bacteria - all years MgmtDate, include=FALSE}
...
theme(panel.background = element_blank()) + 
  ...
  guides(fill = guide_legend(override.aes = list(pch=21, size = 4, colour="black")),
         ...
         annotate("text", -Inf, Inf, label = " bacteria", hjust = 0, 
                  vjust = 2)
...
```{r Ordination (PCoA) - Bacteria - all years MgmtPl, include=FALSE}
...
```{r Ordination (PCoA.2011) - Bacteria, echo=TRUE}
new.data <-cbind(design,dataREL)
# PERMANOVA - 2011 only
...
plot8a <- ggplot(df8a, aes(x=V1, y=V2, colour=pcoa.col, shape = pcoa.shape,
                 group = interaction(pcoa.col, pcoa.shape))) + theme_bw() 
bact.2011.legend<- plot8a + theme(panel.grid.major = element_blank(), 
               panel.grid.minor = element_blank(), 
               axis.line = element_line(colour = "black")) + 
theme(panel.background = element_blank()) + 
  geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.01), colour="black") +    
...
```{r Ordination (PCoA.2012) - Bacteria, echo=TRUE}
# PERMANOVA - 2012 only
...
```{r Ordination (PCoA.2013) - Bacteria, echo=TRUE}
# PERMANOVA - 2013 only
...
```{r Ordination (PCoA.combined) - Bacteria, echo=TRUE}
...
```{r perMANOVA - Nematodes - All Yrs, echo=TRUE}
#PERMANOVA Date|Plant|Management
str(nemaREL)
...
## Principal Coordinates Ordination
```{r Ordination (PCoA) - Nematodes, include=FALSE}
...
```{r Ordination (All Yrs) - Nematodes MgmtDate, include=FALSE}
# Principal Coordinates Analysis Mgmt*Date
...
theme(panel.background = element_blank()) + 
  ...
  guides(fill = guide_legend(override.aes = list(pch=21, size = 4, colour="black")),
         shape = guide_legend(override.aes = list(size = 4, fill="black"))) + 
        annotate("text", -Inf, Inf, label = " nematodes", hjust = 0, 
                  vjust = 2)
...
```{r Ordination (All Yrs) - Nematodes MgmtPlant, include=FALSE}
# Principal Coordinates Analysis (Mgmt*Plant)
...
```{r Ordination (PCoA.2011) - Nematodes, echo=TRUE}
...
```{r Ordination (PCoA.2012) - Nematodes, echo=TRUE}
...
```{r Ordination (PCoA.2013) - Nematodes, echo=TRUE}
...
```{r Ordination (PCoA.combined) - Nematodes, echo=TRUE}
#combine bacteria 2011,2012,2013
library(gridExtra)
library(cowplot)
...
```{r Nematode Indicator Species, include=FALSE}
nemaREL <- cbind(nema.design,dataRELnema)
library("labdsv")
